...
describe("canonical x402 USDC identities", () => {
  it("publishes checksummed token addresses and canonical signing domains", () => {
    ...
    expect(ARC_MAINNET_CAIP2).toBe("eip155:5042");
    expect(getCanonicalX402Usdc(ARC_MAINNET_CAIP2)).toBeDefined();
  ...
  it("deep-freezes token, domain, and browser-enablement authority", () => {
    ...
    const browserArcMainnet = BROWSER_ENABLED_X402_NETWORK_CONFIG[ARC_MAINNET_CAIP2]!;
    ...
    expect(BROWSER_ENABLED_X402_NETWORK_CONFIG[ARC_MAINNET_CAIP2]?.enabled).toBe(true);
    expect(Object.isFrozen(browserArcMainnet)).toBe(true);
    expect(Object.isFrozen(browserArcMainnet.eip712Domain)).toBe(true);
